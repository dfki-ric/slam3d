...
namespace slam3d {
    ...
    RedisMeasurementStorage::RedisMeasurementStorage(const char *ip, int port, size_t cacheSize, bool useBinaryArchive):MeasurementStorage(),cacheSize(cacheSize),useBinaryArchive(useBinaryArchive) {
    ...
    void RedisMeasurementStorage::store(const std::string& key, const std::string &type, const std::string& serializedData) {
        ...
        void* reply = redisCommand(context.get(), "HSET %s type %s data %b", key.c_str(), type.c_str(), serializedData.data(), serializedData.size());
        if (!reply) {
