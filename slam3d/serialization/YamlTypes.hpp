...
namespace YAML
{
	...
	template<> struct convert<slam3d::Constraint::Ptr>
	{
		...
		static Node encode(const slam3d::Constraint::Ptr& config)
		{
			...
			if (!node)
			{
				throw std::runtime_error("Could not load constraint of sensor: " + config->getSensorName());
			}
			return node;
		}
	};
	...
	template<> struct convert<slam3d::MetaData>
	{
		...
		{
			checkAndSet(&meta.timestamp, node["timestamp"]);
			checkAndSet(&meta.robotName, node["robotName"]);
			checkAndSet(&meta.sensorName, node["sensorName"]);
		...
		static Node encode(const slam3d::MetaData& meta)
		{
			...
			node["robotName"] = meta.robotName;
			node["sensorName"] = meta.sensorName;
	...
	{
		static bool decode(const Node& node, slam3d::VertexObject& vertex)
		{
			checkAndSet(&vertex.measurement, node["measurement"]);
			checkAndSet(&vertex.index, node["index"]);
			checkAndSet(&vertex.label, node["label"]);
			checkAndSet(&vertex.correctedPose, node["correctedPose"]);
			checkAndSet(&vertex.fixed, node["fixed"]);
			return true;
		}
		static Node encode(const slam3d::VertexObject& vertex)
		{
			Node node;
			node["measurement"] = vertex.measurement;
			node["index"] = vertex.index;
			node["label"] = vertex.label;
			node["correctedPose"] = vertex.correctedPose;
			node["fixed"] = vertex.fixed;
			return node;
		}
	};
}
